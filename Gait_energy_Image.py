...
for k in range(1, 6):
        ...
        path = glob.glob('./GaitDatasetB-silh/00{k}/nm-03/108/*')  # path = glob.glob('GaitDatasetB-silh/001/001/bg-01/090/001-bg-01-090-.png') # Path of the frames
        ...
        for path in glob.glob('./GaitDatasetB-silh/00{k}/nm-03/108/*'):  # For every frame in the path
        ...
        cv2.imwrite('GEI.png', GEI)  # Write the image
        GEI = cv2.imread('GEI.png')
        GEI.save(f'./Extracted_Silhouette01/00{k}/nm-03/108/img{k}.png')
# # Visualising the result
# x, X, y, Y = Border(GEI)  # Find the borders of the GEI
# GEI = Crop(GEI, x, X, y, Y)  # Crop the GEI
# cv2.imwrite('GEI.png', GEI)  # Write the image
# GEI = cv2.imread('GEI.png')  # Read the image
